...
class Sher_Core_Util_Image {
	...
	public static function make_photo($path,$scale_width=480){
		...
		$result['photo'] = self::genPath($path, 'photo');
		Sher_Core_Util_Asset::storeData('sher', $result['photo'], $bytes);
		
		$gmagick->destroy();
		
		$result['photo_url'] = Sher_Core_Helper_Url::asset_view_url($result['photo']);
     ...
     * @param int $fileName
     */
    public static function genPath($fileName, $prefix='photo'){
		$fs_id = new MongoId();
